...
/*
// Allow python to load precompiled shared libraries. This is really icky code.
...
static struct dlinfo *opened = NULL;
static char dlerror_buf[128] = {0};
static int dlerror_inc = 0;
static int dlerror_call = 0;

char *dlerror()
{
	INIT_NEXT("dlerror", char *, void);
	PUNT_NEXT();

	if (dlerror_call == dlerror_inc)
		return next();
	else
		++dlerror_call;
...
{
	INIT_NEXT("dlopen", void *, const char *, int);
	PUNT_NEXT(filename, flag);

	if (filename == NULL)
		return next(filename, flag);

	void *handle = next(filename, flag | RTLD_NOLOAD);
	if (handle != NULL)
		return handle;
}
...
{
	static void *(*next)(void *, const char *) = NULL;
	if (next == NULL) {
		next = _dl_sym(RTLD_NEXT, "dlsym", dlsym);
	}

	// if handle is ours, it is a pointer to our opened list
	// if it is not ours, pass the call to the real dlsym
	if (opened == NULL)
		return next(handle, symbol);

	struct dlinfo *comp;
	for (comp = opened; comp != handle && comp != NULL; comp = comp->next);
	if (comp == NULL)
		return next(handle, symbol);

	// if we get here, it's something we loaded, meaning we now need to find that
	// symbol name in comp->addr somewhere
}
*/
